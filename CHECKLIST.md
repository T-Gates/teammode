...
- [ ] **W-D verify + SPEC** — 모킹-윈도우 골든/e2e(install→on→context→uninstall 라운드트립을 nt 모킹). SPEC §9(env)·§2(어댑터 명령)에 Windows 반영. 부록 A.3 "Windows 미지원" 제거. 합격=모킹 윈도우 green + Linux 301 무회귀. 실 윈도우 검증은 은수 내일 표기.
