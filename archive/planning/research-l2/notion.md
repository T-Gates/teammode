...
**팀 실제 등록 형식 (참고 — T-gates `notion-tgates`)**: 토큰을 JSON에 박지 않고 `.env` + start.sh 래퍼로 분리.
- 등록: `command` = `.../infra/mcp/notion-tgates/start.sh`, `args` = `[]`, `env` = `{}`
...
- 팀 실제 설정: `/home/euns/work/soma/tgates/tgates-toolkit/infra/mcp/notion-tgates/` (start.sh, .env.example, package.json)
