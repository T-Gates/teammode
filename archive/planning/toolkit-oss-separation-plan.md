# tgates-toolkit 오픈소스 분리 계획 (Plan 에이전트 보고서)
...
추천 경로: **private 내부에 `team.config.json` 설정 레이어 먼저 도입(무중단) → 엔진만 fresh-start public 레포 추출 → T-Gates는 upstream remote merge로 엔진 업데이트 수신.**
...
| `CLAUDE.md:111`, `AGENTS.md:102` | "Bot: T-Gates 토깽이" | [C] `slack.bot_name` |
| `infra/cron/daily-digest.sh:16` | `DAILY_CHANNEL:-C0B71DG9E76` 기본값 | [C] (env 폴백 유지) |
| `infra/skills/base/tgates-schedule/SKILL.md:67` | #공지사항 ID 자동 발송 | [C] 참조로 치환 |
| `infra/skills/core/tgates-manage-knowledge/SKILL.md:111,117,136,155` | #지식베이스 `C0B91G9NN0K` ×4 | [C] `slack.channels.knowledge` |
| `infra/mcp/slack-tgates/server.mjs:43` | 설명 텍스트의 예시 채널 ID | [E] |
...
| `infra/skills/util/tgates-credentials/SKILL.md:12,16,22` | Secret 페이지·계정 DB·Key DB ID | [C] |
| `infra/skills/core/tgates-create-meeting/SKILL.md:12` | 문서 DB ID | [C] |
| `infra/skills/base/tgates-onboard/SKILL.md:213` | Key DB ID | [C] |
...
| 파일:라인 | 내용 | 이동처 |
|---|---|---|
| `CLAUDE.md:87-88`, `AGENTS.md:81-84` | `linear.app/tgates/team/T/active`, prefix `TG` | [C] |
| `infra/skills/core/tgates-create-tasks/SKILL.md:28-30` | 워크플로 상태 UUID 3개 | [C] `linear.states` |
...
| `infra/skills/util/tgates-lint/SKILL.md:53,103` | prefix 통일 검사 | 설정값 대조로 |
...
| `infra/hooks/session-log-remind.py:40,53` | `eunsu/junhyeong/xian` 하드코딩 (members.md 단일 참조 원칙 위반) | members.md 동적 로드 또는 일반 문구 |
| `infra/skills/base/tgates/SKILL.md:55,114` | 멤버 이모지, "은수 daily 로그 밀도가 기준" | members.md 참조로 |
| `infra/skills/core/tgates-get-context/SKILL.md:34-48` | 이모지·이름 출력 예시 | [E] |
| `infra/skills/base/tgates-onboard/SKILL.md:50` | "CTO(은수)에게 받거나" | [C] `admin_contact` |
| `infra/cron/daily-digest.sh:7`, `SPEC.md:601` | "은수 라즈베리파이" | [E] "운영 호스트" |
...
| 항목 | 처리 |
|---|---|
| "그린고래/greengorae", "소마 17기" (CLAUDE.md, INDEX.md, SPEC.md 등) | [C] `product_name`, `team_description` |
| tempio 예시들 | [E] |
| `infra/skills/util/tgates-soma-browse/` 전체 | [P] public 제외 |
| 캘린더 카테고리(소마/팀/개인) + colorId 매핑 | [C] `calendar.categories` |

### 1-6. 환경변수·기타

| 항목 | 처리 |
|---|---|
| `TGATES_HOME` (156곳) | **유지 권장** — rename은 전 팀원 재설치 유발, 이득 없음 |
| `TGATES_GCAL_ID` (8곳) | [C] `gcal.key_name` |
| `tgates on/off`, `tgates-` 스킬 접두사 | v1 유지 (rename은 메이저 버전 과제) |
| T-Gates ASCII 아트 배너 | [C] 또는 persona 파일로 |
...
| `infra/skills/util/tgates-lint` | decisions/current.md 등 구조 검사 |
...
| 1 | team.config.json 도입 + [C] 치환(~40개소) + session-log-remind 이름 하드코딩 제거 | tgates-lint 확장 + on/off 왕복 + digest 수동 1회 + 스킬 3종 스모크 |
| 2 | 경계 정리 (soma-browse 표시, SPEC 분리, CLAUDE.md 재구성) | lint + 마이그레이션 0006 추가 |
...
- `tgates-` 네이밍: v1 유지, rename은 v2
- 미결: 한/영 수준·linear-mcp 패치본(@hatcloud) 라이선스 확인
- ~~레포 이름~~ → **`teammode` 확정 방향 (2026-06-11, 은수 발안 "팀모드 툴킷")** — 이름=사용법(`teammode on`), 태그라인 "Turn your team mode on." 약점: 일반명사 SEO 불리(직링크 유통이라 수용). GitHub 선점 확인 필요
...
| 4. 일상 사용 | 전원 | — | tgates on → 훅이 세션로그 반강제 누적 → 2주차 복리 시작 |
| 5. 엔진 업데이트 | 리더 | 월 1회 | `git fetch upstream && git merge upstream/main` |
| 6. 커스텀 | 팀 | — | 팀 전용 util 스킬 추가, 잘 만든 건 upstream PR 역기여 |

설계 포인트:
- **⚠️ 도입 경로 이원화 (2026-06-11 발견)** — 도입 팀은 **fork 금지, Use this template(→ org private) 또는 clone+private push**로. public 레포의 포크는 무조건 public이라 팀 데이터(회의록·세션로그)가 공개되는 함정. upstream 업데이트는 GitHub 포크 관계 없이도 `git remote add upstream`+merge로 수신 가능(T-Gates 자신이 이 방식). fork는 기여자(PR) 전용. README 온보딩에 경고 박스 1순위
- **성공 지표 — "스타 100 + 살아있는 팀 5"** (런칭 1~2달): ① 도입 팀 3~5 ② 2주 생존율(세션로그 지속) 절반↑ ③ 외부 기여자 2~3 ④ 스타 50~100. 활성 팀 > 스타. 확산 동선: 1팀(친구) 검증 → 저변 확대 → 웹엑스(소마 전체) 공유
...
1. **마법의 순간을 팔기** — README 최상단 30초 데모 GIF: "지금 팀 상황" → 팀원 3명 작업·블로커·일정 좌르륵. 실화 소재: 은수의 질문(센서 1:1)과 준현의 플래그(nodes.* stale)가 같은 지점임을 시스템이 잡아낸 장면
...
4. **정직한 기대치** — "우리도 첫 주는 귀찮았다. 2주차부터 시스템이 우리보다 팀을 잘 기억했다" (T-Gates 실경험 = 1호 고객 후기)
...
- **2-웨이브 릴리즈**: ① 7월 초중순 1차 발사("소마 3인 팀이 6주째 실전 사용") — 선점 리스크(유사 툴킷·네이티브 기능) 때문에 소마 종료까지 안 기다림 ② 소마 종료(11~12월) 시 "풀시즌 데이터 회고" 글로 2차 점화
- **홍보 채널**: 긱뉴스·디스콰이엇·기술블로그(1군) → X·Reddit r/ClaudeAI·awesome 리스트(2군) → 소마 내부는 격차 가치 줄어든 뒤(3군)
- **선행 조건**: ~~팀 합의~~ ✅ **완료 (2026-06-11 — 준형·준현 긍정적)**, Notion 토큰 회전(보안 7절)만 남음
...
- **YC 각도 (2026-06-11, 은수)**: teammode 평판 → 추후 YC 지원 시 founder signal로 활용 구상. 위치: 본업(그린고래) 지원서의 보조 증거 — "자체 협업 인프라를 만들어 오픈소스화, N팀 사용"은 빌딩 속도·distribution 능력 증명. teammode 자체로 YC는 비추(제품화 기각 논리 동일 — 플랫폼 리스크)
- **레포 소유·권한 (2026-06-11)**: 새 org 안 팜. T-Gates org 아래 public 레포(평판이 팀에 쌓임) + 친구는 **그 레포만 outside collaborator(maintain)** — org 멤버 초대 금지(private 레포 노출 위험). org base permission "No permission" 확인. 프로젝트가 커지면 그때 전용 org로 transfer(스타·리다이렉트 보존됨)
...
- 역할: 은수 lead maintainer(머지 권한), 협업자는 contributor→maintainer 승급. 에이전트별 담당 메인테이너 체제
...
- 스킬 작성 규약(짧은 명령문·번호 단계·명시적 출력 포맷·에이전트 전용 문법 금지) + tgates-lint에 이식성 검사 추가
- 갈라질 땐 어댑터: 공통 본문 + 에이전트별 분기 블록 (전체 복제본 2개 금지 — 드리프트 지옥)
- 골든 시나리오 5개(켜기→컨텍스트→이슈→로그→끄기) 릴리즈 전 에이전트별 수동 체크
- **훅 어댑터 레이어 (은수 발안)**: manifest.json에 `agents` 필드(지원 에이전트) + `event` 에이전트별 매핑 추가 → sync.py가 플래그(--claude/--codex/--hermes) 보고 자기 것만·자기 이벤트명으로 등록. 대응 이벤트 없으면 해당 훅만 우아한 축소. Hermes 타겟 = ~/.hermes/plugins (pre_llm_call≈UserPromptSubmit, on_session_start≈SessionStart)
...
- `infra/skills/util/tgates-lint/SKILL.md` — 각 단계 검증 게이트
