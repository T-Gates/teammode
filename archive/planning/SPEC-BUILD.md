...
1. `/home/euns/work/soma/tgates/teammode/toolkit-oss-separation-plan.md` — 프로젝트 전체 계획
2. `/home/euns/work/soma/tgates/teammode/teammode-adapter-spec-draft.md` — 어댑터 스펙 초안 (오늘 밤 설계의 본체)
3. `/home/euns/work/soma/tgates/teammode/team.config.example-draft.json` — config 스키마 초안
4. `/home/euns/work/soma/tgates/tgates-toolkit/memory/team/sessions/eunsu/2026-06-11.md` — 설계 세션 로그 (특히 "teammode" 관련 항목 전부)
5. `/home/euns/work/soma/tgates/tgates-toolkit/CLAUDE.md` — 현행 툴킷 구조
6. `/home/euns/work/soma/tgates/tgates-toolkit/infra/hooks/manifest.json` + `sync.py` — 현행 훅 실물
...
3. **`spec/03-conformance.md`** — 호환 선언 절차
   - 독립 구현이 "teammode 호환"을 선언하는 조건, conformance kit 구상(tgates-lint 일반화), Implementations 등재 절차, 배지
...
추가로: 정직한 기대치(첫 주는 귀찮음, 2주차부터 복리 — T-Gates 실사용 6주 스토리), 우리가 매일 쓰는 물건이라는 점(독푸딩), 설치 흐름 3줄 예시.
...
- 메인·검증 서브에이전트 전부 **모델 지정 금지** — 세션 기본값(Fable)을 그대로 상속한다 (은수 지정: "전부 페이블").

## 마감 규칙

- 파일 작성은 Read/Write/Edit만 사용. git 커밋 금지(은수가 검토 후 직접).
