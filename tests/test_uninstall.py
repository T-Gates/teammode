...
현 코드베이스의 실제 설치 산물(.tgates-active 마커 + settings.json teammode 훅)에
...
def _profile_with_ours(tmp_path):
    ...
    p.write_text(
        ...
        f'export TGATES_TEAM_NAME=tgates  {ENV_MARKER} v0.1)\n'
...
def test_remove_injected_env_only_our_line(tmp_path):
    ...
    assert "TGATES_TEAM_NAME" not in text
    # 남의 줄은 전부 보존
...
def test_uninstall_idempotent(tmp_path):
    ...
    assert not (team_root / ".tgates-active").exists()
