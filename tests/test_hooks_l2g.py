...
안전 철칙(반드시):
  ...
  - auto-commit 의 `.tgates-active` 가드가 "마커 없으면 no-op"임을 실증(빌드 오염 0).
...
def test_auto_commit_no_marker_is_noop(fake_repo):
    """빌드 안전 핵심: .tgates-active 없으면 절대 커밋하지 않는다(no-op exit 0)."""
...
def test_auto_commit_active_commits(fake_repo):
    """.tgates-active 있으면 발동 — 지목 파일이 커밋된다."""
    (fake_repo / ".tgates-active").write_text("")
...
def test_auto_commit_nonblocking_on_git_failure(tmp_path):
    ...
    plain.mkdir()
    (plain / ".tgates-active").write_text("")
...
def test_confirm_no_marker_is_noop(fake_repo):
    """빌드 안전: .tgates-active 없으면 차단도 안 함(exit 0)."""
    payload = {"event": "PreToolUse",
               "tool": {"kind": "mcp", "server": "linear", "name": "create_issue"},
               "agent": "claude"}
    proc = _run_hook(CONFIRM, payload, fake_repo, args=["tgates-linear-create-allow"])
    assert proc.returncode == 0
...
def test_confirm_allows_with_env_signal(fake_repo):
    ...
    payload = {"event": "PreToolUse",
               ...
               "agent": "claude"}
    proc = subprocess.run(
        [PY, str(CONFIRM), "tgates-linear-create-allow"],
        input=json.dumps(payload), capture_output=True, text=True,
        env={**os.environ, "TEAMMODE_HOME": str(fake_repo),
             "TEAMMODE_CONFIRM": "tgates-linear-create-allow"})
    assert proc.returncode == 0
...
def test_confirm_allows_with_fresh_signal_file(fake_repo):
    ...
    confirm_dir.mkdir()
    (confirm_dir / "tgates-linear-create-allow").write_text("")
    payload = {"event": "PreToolUse",
               "tool": {"kind": "mcp", "server": "linear", "name": "create_issue"},
               "agent": "claude"}
    proc = _run_hook(CONFIRM, payload, fake_repo, args=["tgates-linear-create-allow"])
...
def test_confirm_stale_signal_file_still_blocks(fake_repo):
    ...
    confirm_dir = fake_repo / ".teammode-confirm"
    confirm_dir.mkdir()
    flag = confirm_dir / "tgates-linear-create-allow"
    flag.write_text("")
    ...
    **여전히 차단**(exit 2). allow 판정은 모델 비제어 채널만 보므로 우회 불가."""
    ...
    payload = {
        ...
        "raw": {
            ...
            "tool_input": {
                "title": "see tgates-linear-create-allow ticket",
                "description": "tgates-linear-create-allow tgates-linear-create-allow",
                "note": "tgates-linear-create-allow",
            },
        },
    }
    proc = _run_hook(CONFIRM, payload, fake_repo, args=["tgates-linear-create-allow"])
...
def test_confirm_bad_stdin_no_block(fake_repo):
    ...
    (fake_repo / ".tgates-active").write_text("")
    proc = subprocess.run(
        [PY, str(CONFIRM), "tgates-linear-create-allow"], input="bad{",
