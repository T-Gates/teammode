...
def test_verify_does_not_touch_ambient_host(tmp_path, monkeypatch):
    ...
    (fake_host / ".tgates-active").write_text("")
    ...
    assert (fake_host / ".tgates-active").exists(), "실호스트 ON 마커가 삭제됨 — 격리 실패!"
...
def test_conform_also_isolated(tmp_path, monkeypatch):
    ...
    assert (fake_host / ".tgates-active").exists(), "conform 이 실호스트 마커 삭제 — 격리 실패!"
...
def test_direct_off_ignores_ambient_team_home(tmp_path):
    ...
    피해자 경로의 .tgates-active 는 생존한다 (엔진이 env 를 안 읽음)."""
    victim = tmp_path / "VICTIM"
    victim.mkdir()
    (victim / ".tgates-active").write_text("")
    ...
    assert (victim / ".tgates-active").exists(), (
...
def test_direct_on_with_root_writes_only_to_root(tmp_path):
    ...
    assert proc.returncode == 0, proc.stderr
    assert (run_root / ".tgates-active").exists()
...
def test_settings_omitted_without_install_refuses(tmp_path):
    ...
    assert not (run_root / ".tgates-active").exists()
