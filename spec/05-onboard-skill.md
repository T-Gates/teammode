...
| 상태 | 설계 초안 (2026-06-14 새벽, 은수 "어떻게 구현할지 생각해봐 / 낼 보러와야지"). **미검수** — 낼 dev-cycle 문서모드로 적대 검수 예정. |
| 범위 | `tm-onboard` 스킬 = 온보딩의 **판단층(LLM)**. install.py(기계층)를 지휘 + 서비스 연결을 대화로 안내. |
| 관련 | [04 install.py](04-install.md), [02 훅·어댑터](02-hook-manifest.md) §9 역할슬롯, prior art: 툴킷 `infra/skills/base/tgates-onboard/SKILL.md` |
...
- "Linear 키 어디서 발급?" = 은수가 한참 헤맨 그 마찰 → 에이전트가 정확한 링크·버튼으로 안내해야 (대화).
- OAuth "허용" 클릭 유도 = 대화.
...
각 서비스마다 스킬이 **정확한 링크·버튼**으로 사람을 데려간다(막연한 "키 찾아와" 금지 — 은수가 헤맨 마찰). 토큰 받으면 **API로 리소스 ID 자동조회 → 후보 제시 → 사람 1클릭 선택 → config 기록.**
...
- prior art(tgates-onboard Phase 2)는 검증까지 했으나, teammode는 **검증·자가진단을 별도 `doctor`**(나중 슬라이스)로 분리. tm-onboard는 "셋업+연결+첫가치"까지만. (단일책임.)
...
## 9. prior art 대비 개선점 (툴킷 tgates-onboard → tm-onboard)

| 툴킷 tgates-onboard | teammode tm-onboard |
|---|---|
