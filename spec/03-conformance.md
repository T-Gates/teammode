...
> 상태: v0.1 시점에서 kit은 **구상 단계**다. reference 구현의 lint 스킬(tgates-lint)이 수행하는 구조 검사를 일반화·독립 실행화하는 방향으로 만든다. kit이 공개되기 전까지 호환 선언은 §4의 수동 리뷰로 갈음한다.
...
- **kit = lint의 일반화**: reference 구현은 이미 구조 검사 스킬(tgates-lint)로 자기 레포를 검사한다. 같은 검사를 픽스처 기반·구현 중립으로 추출하면 conformance kit이 된다 — 새 발명이 아니라 기존 자산의 추출.
