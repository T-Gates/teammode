...
- **org/레포 위치**(도입자가 레포를 **새로 만드는** 경우만): 어느 GitHub org·계정에 만들지 **반드시 확인**한다 — 개인 계정 vs 팀 org(예: `T-Gates`). 임의 선택 금지. `gh repo create`/template 단계라 install 이전이지만, 온보딩이 레포 생성부터 하면 여기서 같이 확인한다.
