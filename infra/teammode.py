...
context/issue/log 동사는 후속 슬라이스 (현재는 미구현 → 해당 시나리오 RED 유지).
  ...
  teammode.py on    팀 모드 켜기  — 배너 출력 + 어댑터 sync --on + .tgates-active 생성
  teammode.py off   팀 모드 끄기  — 어댑터 sync --off + .tgates-active 제거
"""
...
def _team_root() -> Path:
    ...
    """
    return Path(os.environ.get("TGATES_HOME", os.getcwd())).resolve()


def _active_marker() -> Path:
    return _team_root() / ".tgates-active"
