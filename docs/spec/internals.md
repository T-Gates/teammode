...
|---|---:|---|---|---|
| `session-start.py` | ✅ `SessionStart` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.tgates-active` 부재, engine import/수집 실패면 no-op | 활성 시 Claude additionalContext JSON stdout, 항상 exit 0 의도 |
| `session-log-remind.py` | ✅ `UserPromptSubmit` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.tgates-active` 부재면 no-op. 활성 시 auto-pull 후 로그 age/프롬프트 카운터 판정 | 필요 시 additionalContext JSON stdout, 정상 exit 0. 단 counter 파일 write 실패는 현재 main에서 별도 catch하지 않음 |
