# handlers/ 규약 (legacy spec)

> **Legacy notice (2026-06-21):** L2 설계는 `docs/BACKLOG.md`의
> "표준 인터페이스→표준 툴셋" 결정에 따라 벤더 MCP 직접 등록기 방향으로 전환됐다.
> 이 문서의 `handlers/` + `infra/mcp/role_server.py` 모델은 v0.2 이전 설계 보존용이다.
> 새 Codex/Claude L2 배선은 이 역할 추상화 위에 구현하지 않는다.

**이전 확정 규약** — 이 문서는 v0.2 이전 팀 핸들러 코드의 계약·인터페이스·보안
규칙을 보존한다. 현재 L2 신규 구현의 단일 소스는 위 legacy notice와 BACKLOG 결정이다.
