...
**(c) 화면** — 셸 프롬프트뿐. teammode 흔적 없음(`team.config.json`·`memory/`·`.tgates-active` 전부 없음).
...
```
**화면** — `state=on`(verify 가 `on` 을 켰으므로 `.tgates-active` 마커 존재), `members` 에 alice 1명, summary 는 없음(로그 0):
```json
